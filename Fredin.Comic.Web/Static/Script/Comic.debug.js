...
Application.prototype={options:{appId:null,permissions:"",cookieSupport:true,useXfbml:false,user:null,menuSelected:-1,baseHref:"/",facebookBaseHref:"/",requireConnect:false,themeBase:"",fbCanvas:false},init:function(){var self=this;
console.info("Initializing application");
console.warn(this.options);
if(this.options.fbCanvas===true){try{console.log(top.location);
if(top.location.href==window.location.href){console.log("matches...");
...
if(this.options.fbCanvas===true){FB.Canvas.setSize();
...
$.ajax({dataType:"json",type:"POST",url:this.options.baseHref+"Error/LogError",data:$.postify({x:message}),error:function(xhr,textStatus,x){}});
...
self.loadState();
if(self.options.comic.ComicId!=null){self.loadComic();
}});
...
$("#wizardContent").wizard({width:734});
$("#publishDialog").dialog({autoOpen:false,buttons:{Publish:function(){self.publishComic();
},Cancel:function(){$(this).dialog("close");
}},closeOnEscape:true,draggable:false,modal:true,resizable:false,width:500});
$(".ui-comic-publish").button({icons:{primary:"ui-icon-signal-diag"}}).click(function(){self.showPublishComic();
});
...
}},loadComic:function(){var self=this;
$.each(this.options.templates,function(t,template){if(self.options.comic.Template.TemplateId==template.TemplateId){$("#templateSelector").select2("select",t);
}});
...
$(".ui-comic-canvas").css("width",this.options.comic.Template.Width+"px").css("height",this.options.comic.Template.Height+"px");
...
}}).focus().parent().css("left",bubble.X+"px").css("top",bubble.Y+"px").draggable({stop:function(event,ui){var element=$("textarea",this);
var b=self.options.comic.Bubbles[element.data("index")];
if(b){b.X=ui.position.left+parseInt(element.css("margin-left"));
...
},showPublishComic:function(){$("#comicTitle").val(this.options.user.Name!=""?this.options.user.Name+"'s Comic Mashup":"My Comic Mashup");
var description="";
this.options.comic.Bubbles.each(function(index,bubble){if(description==""){description=bubble.Text;
}});
...
var data={remixComicId:this.options.comic.ComicId!=null&&this.options.comic.ComicId>0?this.options.comic.ComicId:null,templateId:this.options.comic.Template.TemplateId,frames:[],bubbles:[]};
this.options.comic.Photos.each(function(index,photo){data.frames.push({PhotoId:photo.PhotoId});
});
this.options.comic.Bubbles.each(function(index,bubble){data.bubbles.push({TextBubbleDirectionId:bubble.TextBubbleDirection.TextBubbleDirectionId,Text:bubble.Text,X:bubble.X,Y:bubble.Y});
});
var publish=function(){var publishData={comicId:self.options.comic.ComicId,title:$("#comicTitle").val(),description:$("#comicDescription").val(),isPrivate:$("[name=optionPrivacy]:checked").val()==="Friends"};
console.log("Publishing comic "+self.options.comic.ComicId);
$.ajax({dataType:"json",type:"POST",url:self.options.baseHref+"Comic/Publish",data:$.postify(publishData),success:function(data,textStatus,request){if(data){self.options.comic=data;
console.log("Published comic "+data.ComicId);
FB.ui({method:"feed",name:self.options.comic.Title,description:self.options.comic.Description,picture:self.options.comic.FrameThumbUrl,link:self.options.comic.ReadUrl,actions:[{name:"Remix",link:self.options.comic.RemixUrl}]},function(response){document.location=data.ReadUrl;
});
...
$.ajax({dataType:"json",type:"POST",url:this.options.baseHref+"Comic/QueueRenderAdvanced",data:$.postify(data),success:function(data,textStatus,request){if(data){task=data;
...
$("#wizardContent").wizard({validator:validator,width:734,stepActive:function(event,data){if(data&&data.step==2){var count=self.options.selectedComments.length+1;
...
$("#buttonEdit").button({icons:{primary:"ui-icon-pencil"}}).click(function(){document.location=self.options.comic.RemixUrl;
});
...
if(!templateId||templateId==""){templateId=22;
}var data={effect:$("#effectSelector").val(),photoSource:$("[name=optionPhoto]:checked").val(),templateId:templateId,frames:[{message:this.options.selectedStory.message,id:this.options.selectedStory.from.id}]};
...
$.ajax({dataType:"json",type:"POST",url:this.options.baseHref+"Comic/QueueRenderWizard",data:$.postify(data),success:function(data,textStatus,request){if(data){task=data;
...
progressInterval=window.setInterval(progressCallback,progressFrequency);
},shareFeed:function(){FB.ui({method:"feed",from:this.options.user.uid,description:"I just used Comic Mashup for Profiles to transform my profile picture into a comic!",name:"Comic Mashup Profile Photo",caption:"http://apps.facebook.com/comicmashup/",picture:this.options.task.RenderUrl,link:"http://apps.facebook.com/comicmashup/Facebook/"});
...
(function($){if(typeof User=="undefined"){User={};
}User.Settings=function(options){Application.create({options:{},init:function(){var self=this;
Application.prototype.init.apply(this);
$(".ui-optionset").optionset();
var subscribeChange=function(){if($("#Engage_Subscribe").is(":checked")){$("#engage-setting").show(200);
}else{$("#engage-setting").hide(200);
}};
$("#Engage_Subscribe").change(subscribeChange);
subscribeChange();
var validator=$("form","#settings").validate({debug:false,errorClass:"ui-state-highlight",rules:{Email:{required:true,email:true},},messages:{Email:{required:"Please enter an email address",email:"Invalid email address"}}});
}},options);
};
}(jQuery));
