...
(function($){$.widget("ui.bubble",{options:{bubble:null,allowDelete:false,onDelete:null},_create:function(){var self=this;
var change=function(){var text=$(this).val().replace(/<br\/>/ig,"\n").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/\r?\n/ig,"<br/>");
var sizer=$(".ui-bubbleSizer",$(self.element).parent()).html(text);
$(this).css("width",sizer.width()+10+"px").css("height",sizer.height()+"px");
...
$(".ui-bubbleImage",$(self.element).parent()).css("width",width+"px").css("height",height+"px");
...
$(".ui-bubbleDelete",$(self.element).parent()).css("left",offsetX+sizer.width()+10+"px").css("top",(height/2)-4+"px");
};
var click=function(){$(self.element).focus();
};
var kill=function(){console.log("killing machine!");
self._trigger("onDelete");
$(self.element).parent().remove();
self.destroy();
};
$(this.element).wrap("<div class='ui-bubble'></div>").addClass("ui-bubbleText").keydown(change).keyup(change).change(change);
$(this.element).parent().mouseup(click).append("<img class='ui-bubbleImage' src='"+self.options.bubble.ImageUrl+"' />").append("<div class='ui-bubbleSizer'></div>");
if(this.options.allowDelete){$(this.element).parent().append("<a class='ui-bubbleDelete' href='javascript:void(0);'></a>");
$(".ui-bubbleDelete",$(this.element).parent()).click(kill);
}$(this.element).change();
}});
}(jQuery));
...
(function($){$.widget("ui.wizard",{options:{step:0,width:400,validator:null,stepActive:null,stateField:"step"},_create:function(){var self=this;
...
return true;
},loadState:function(){var step=$.bbq.getState(this.options.stateField,true)||0;
...
(function($){$.fn.tipsy=function(options){options=$.extend({},$.fn.tipsy.defaults,options);
return this.each(function(){var opts=$.fn.tipsy.elementOptions(this,options);
$(this).hover(function(){$.data(this,"cancel.tipsy",true);
var tip=$.data(this,"active.tipsy");
if(!tip){tip=$('<div class="tipsy"><div class="tipsy-inner"/></div>');
tip.css({position:"absolute",zIndex:100000});
$.data(this,"active.tipsy",tip);
}if($(this).attr("title")||typeof($(this).attr("original-title"))!="string"){$(this).attr("original-title",$(this).attr("title")||"").removeAttr("title");
}var title;
if(typeof opts.title=="string"){title=$(this).attr(opts.title=="title"?"original-title":opts.title);
...
switch(gravity.charAt(0)){case"n":tip.css({top:pos.top+pos.height,left:pos.left+pos.width/2-actualWidth/2}).addClass("tipsy-north");
break;
...
break;
}if(opts.fade){tip.css({opacity:0,display:"block",visibility:"visible"}).animate({opacity:0.8});
}else{tip.css({visibility:"visible"});
}},function(){$.data(this,"cancel.tipsy",false);
var self=this;
setTimeout(function(){if($.data(this,"cancel.tipsy")){return;
}var tip=$.data(self,"active.tipsy");
if(opts.fade){tip.stop().fadeOut(function(){$(this).remove();
});
}else{tip.remove();
}},100);
});
...
}Comic.Create=function(options){Application.create({options:{comic:{ComicId:null,Template:null,Bubbles:[],Photos:[]},templates:[],effects:[],bubbleDirections:[],selectedBubbleDirection:null,selectedPhoto:null,activeFrame:0,bubbleIndex:0},init:function(){var self=this;
Application.prototype.init.apply(this);
this.requestConnect(true,"Comic Mashup needs to connect with your facebook account to retrieve your photos.",function(){self.render();
self.loadState();
});
},render:function(){var self=this;
$("#templateSelector").select2({multiple:false,data:self.options.templates,dataKey:"TemplateId",dataValue:"TemplateId",template:$("#templateTemplate"),emptyText:"No templates found.",selected:function(event,data){self.options.comic.Template=data.item;
$("#wizardContent").wizard("step","#stepCreate");
$.bbq.pushState({template:data.item.TemplateId});
self.loadTemplate();
}});
$("#bubbleSelector").select2({multiple:false,data:this.options.bubbleDirections,dataKey:"TextBubbleDirectionId",dataValue:"TextBubbleDirectionId",template:$("#bubbleTemplate"),emptyText:"No text bubbles found.",selected:function(event,data){self.options.selectedBubbleDirection=data.item;
}});
$("#bubbleDialog").dialog({autoOpen:false,buttons:{Add:function(){$(this).dialog("close");
self.addBubble(self.options.bubbleIndex++,{TextBubbleDirection:self.options.selectedBubbleDirection,Text:"",X:0,Y:0});
},Cancel:function(){$(this).dialog("close");
}},closeOnEscape:true,draggable:false,modal:true,resizable:false,width:448});
$("#photoDialog").dialog({autoOpen:false,buttons:{Add:function(){$(this).dialog("close");
self.renderPhoto();
},Cancel:function(){$(this).dialog("close");
}},closeOnEscape:true,draggable:false,modal:true,resizable:false,width:648});
$("#albumSelector").select2({multiple:false,data:[],dataKey:"id",dataValue:"id",template:$("#albumTemplate"),emptyText:"No albums found.",selected:function(event,data){$("#photoLoad").load("reset");
self.loadPhotos(data.item.id,function(photos){$("#photoLoad").load("complete");
$("#photoSelector").select2("option","data",photos);
});
$("#photoWizard").wizard("step","#stepPhoto");
}});
$("#albumLoad").load({content:$("#albumContent")});
this.loadAlbums(this.options.user.Uid,function(albums){$("#albumSelector").select2("option","data",albums);
$("#albumLoad").load("complete");
});
$("#photoSelector").select2({multiple:false,dataKey:"id",dataValue:"id",template:$("#photoTemplate"),emptyText:"No photos found.",selected:function(event,data){self.options.selectedPhoto=data.item;
...
$("#photoWizard").wizard({width:620,stateField:"photoStep"});
$(".ui-comic-bubbleAdd").button({icons:{primary:"ui-icon-comment"}}).click(function(){$("#bubbleDialog").dialog("open");
$("#photoWizard").wizard("step","#stepAlbum");
});
$("#renderDialog").dialog({autoOpen:false,buttons:{},closeOnEscape:false,draggable:false,modal:true,resizable:false,width:300});
$("#wizardContent").wizard({width:764});
$("#wizardContent").show();
},loadState:function(){var self=this;
Application.prototype.loadState.apply(this);
var templateId=$.bbq.getState("template",true);
if(templateId!=null&&(!this.options.comic.Template||templateId!=this.options.comic.Template.TemplateId)){$.each(this.options.templates,function(t,template){if(templateId==template.TemplateId){$("#templateSelector").select2("select",t);
}});
}},loadTemplate:function(){var self=this;
$(".ui-comic-canvas").remove(".ui-comic-frame").css("width",this.options.comic.Template.Width+"px").css("height",this.options.comic.Template.Height+"px");
$.each(this.options.comic.Template.TemplateItems,function(i,item){$(".ui-comic-canvas").append("<div class='ui-comic-frame'></div>");
var frame=$(".ui-comic-frame:eq("+i+")").css("width",item.Width+"px").css("height",item.Height+"px").css("left",item.X+"px").css("top",item.Y+"px");
if(self.options.comic.Photos[i]!=null){self.addPhoto(i,self.options.comic.Photos[i]);
}else{frame.append("<div class='ui-comic-frameButton'><a>Select Photo</a></div>");
$(".ui-comic-frameButton a",frame).button({icons:{primary:"ui-icon-image"}}).click(function(){self.showAddPhoto(i);
});
}});
},loadAlbums:function(uid,callback){var self=this;
$("#albumLoad").load("reset");
FB.api("/"+uid+"/albums",{},function(response){if(!response||response.error){self.error("Unable to retrieve albums from facebook.",response);
}else{callback(response.data);
}});
},loadPhotos:function(albumId,callback){var self=this;
$("#photoLoad").load("reset");
FB.api("/"+albumId+"/photos",{},function(response){if(!response||response.error){self.error("Unable to retrieve photos from facebook.",response);
}else{callback(response.data);
}});
},addBubble:function(index,bubble){var self=this;
this.options.comic.Bubbles[index]=bubble;
$(".ui-comic-canvas").append("<textarea>"+bubble.Text+"</textarea>");
var b=$("textarea",".ui-comic-canvas").not(".ui-bubbleText").data("index",index).change(function(){self.options.comic.Bubbles[$(this).data("index")].Text=$(this).val();
}).bubble({allowDelete:true,bubble:bubble.TextBubbleDirection,onDelete:function(event,data){self.options.comic.Bubbles.splice($(this).data("index"),1);
}}).focus().parent().css("left",bubble.X+"px").css("top",bubble.Y+"px").draggable({stop:function(event,ui){var b=self.options.comic.Bubbles[$("textarea",this).data("index")];
if(b){b.X=ui.position.left;
b.Y=ui.position.top;
}}});
},showAddPhoto:function(index){this.options.activeFrame=index;
$("#photoWizard").wizard("step","#stepAlbum");
$("#photoDialog").dialog("open");
$("button:first",$("#photoDialog").next(".ui-dialog-buttonpane")).button("disable");
},addPhoto:function(index,photo){var self=this;
...
if(imageRatio>=scaleRatio){fitHeight=template.Height;
fitWidth=imageRatio*template.Height;
}else{fitWidth=template.Width;
...
var frame=$(".ui-comic-frame:eq("+index+")").empty().append("<img class='' src='"+photo.ImageUrl+"' alt='' width='"+fitWidth+"' height='"+fitHeight+"' />").append("<div class='ui-comic-frameButton'><a>Change Photo</a></div>");
frame.children("img").css("left",offsetX+"px").css("top",offsetY+"px");
$("a",frame).button({text:false,icons:{primary:"ui-icon-image"}}).click(function(){self.showAddPhoto(index);
});
},renderPhoto:function(){var self=this;
var task=null;
$("#renderDialog").dialog("open");
var data={effect:$("#effectSelector").val(),photoSource:this.options.selectedPhoto.source,intensity:$("#buttonsetIntensity :checked").val()};
var timeout=1000*20;
...
}$.ajax({dataType:"json",type:"GET",url:self.options.baseHref+"Comic/PhotoRenderProgress",data:{taskId:task.TaskId},success:function(data,textStatus,request){if(data){task=data;
...
$.ajax({dataType:"json",type:"POST",url:this.options.baseHref+"Comic/QueuePhotoRender",data:$.postify(data),success:function(data,textStatus,request){if(data){task=data;
...
$("#dialog-authorDelete").dialog({autoOpen:false,buttons:{Cancel:function(){$(this).dialog("close");
},Delete:function(){$(this).dialog("close");
self.deleteComic();
}},closeOnEscape:true,draggable:true,modal:true,resizable:false});
...
},deleteComic:function(){window.location=this.options.baseHref+"Comic/Delete/"+this.options.comic.ComicId;
...
(function($){if(typeof Facebook=="undefined"){Facebook={};
}Facebook.Render=function(options){Application.create({options:{task:null,autoShareFeed:false},init:function(){var self=this;
Application.prototype.init.apply(this);
$("#renderLoad").load({content:"#renderContent"});
$("#renderShareRequest").button({icons:{primary:"ui-icon-comment"}}).click(function(){self.shareRequest();
});
$("#renderShareFeed").button({icons:{primary:"ui-icon-star"}}).click(function(){self.shareFeed();
});
...
$("#renderLoad").load("complete");
if(self.options.autoShareFeed===true){self.options.autoShareFeed=false;
self.shareFeed();
...
progressInterval=window.setInterval(progressCallback,progressFrequency);
},shareFeed:function(){FB.ui({method:"feed",from:this.options.user.uid,description:"I just used Comic Mashup for Profiles to transform my profile picture into something cool!",name:"Comic Mashup Profile Photo",caption:"http://apps.facebook.com/comicmashup/",picture:this.options.task.RenderUrl,link:this.options.facebookBaseHref});
},shareRequest:function(){FB.ui({method:"apprequests",message:"Check out this cool app that transforms your profile picture into something cool.",title:"Comic Mashup Profile Photo"});
}},options);
};
}(jQuery));
