...
Application.prototype={options:{appId:null,permissions:"",cookieSupport:true,useXfbml:false,user:null,menuSelected:-1,baseHref:"/",requireConnect:false,themeBase:""},init:function(){var self=this;
...
FB.api("/me/feed",{limit:150},function(response){if(!response||response.error){self.error("Unable to retrieve feed from facebook.",response);
}else{self.options.feed=[];
for(var i=0;
i<response.data.length;
i++){var item=response.data[i];
if(item.from&&item.from.id==self.options.user.Uid&&item.message&&item.message.trim()!=""){self.options.feed.push(item);
}else{continue;
}item.css="";
