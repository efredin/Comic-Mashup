...
}},requestConnect:function(required,callback){var self=this;
...
$("#publishComic").hide();
this.requestConnect(true,function(){var load=$("#wizardLoad").load("progress",25);
...
$("#storySelector").select2({multiple:false,data:self.options.feed,template:$("#storyTemplate"),dataKey:"id",dataValue:"message",emptyText:"No messages found.",selected:function(event,data){self.options.selectedStory=data.item;
...
var updateComments=function(){var count=self.options.selectedComments.length+1;
...
$("#commentSelector").select2({multiple:true,template:$("#commentTemplate"),dataKey:"id",dataValue:"message",emptyText:"No comments found.",selected:function(event,data){self.options.selectedComments.push(data.item);
updateComments();
...
$("#wizardContent").wizard({validator:validator,width:764,stepActive:function(event,data){if(data&&data.step==3){$("#buttonsetPhoto").buttonset("enable");
...
FB.api("/me/feed",{limit:50},function(response){if(!response||response.error){self.error("Unable to retrieve feed from facebook.",response);
...
this.requestConnect(false,function(){var count=$(".readerCount","label[for="+source.id+"]").text();
if(count=="-"){count=0;
