...
$("#menubar > ul > li").hoverIntent({over:menuOpen,out:menuClose,timeout:500}).each(function(){if($("div",this).size()>0){$("a:first",this).append('<span class="ui-icon ui-icon-triangle-1-s"></span>');
}});
...
}Comic.CreateWizard=function(options){Application.create({options:{step:0,templates:[],effects:[],feed:[],friends:[],selectedStory:null,selectedComments:[],comic:null,taskBaseHref:null},init:function(){var self=this;
...
var taskId=$.Guid.New();
var progressCallback=function(){if($.Guid.IsEmpty(taskId)){window.clearInterval(progressInterval);
}else{$.ajax({dataType:"json",type:"GET",async:true,timeout:10000,url:self.options.taskBaseHref+taskId,success:function(data,textStatus,request){if(data){$("#renderLoad").load("progress",data.Progress/data.Operations);
}},error:function(xhr,textStatus,x){}});
}};
var progressInterval=window.setInterval(progressCallback,10000);
var data={taskId:taskId,effect:$("#effectSelector").val(),photoSource:$("[name=optionPhoto]:checked").val(),templateId:$("#templateSelector").val(),frames:[{message:this.options.selectedStory.message,id:this.options.selectedStory.from.id}]};
this.options.selectedComments.each(function(index,comment){data.frames.push({message:comment.message,id:comment.from.id});
});
console.log(data);
$.ajax({dataType:"json",type:"POST",async:true,url:this.options.baseHref+"Comic/RenderWizard",data:$.postify(data),success:function(data,textStatus,request){if(data){self.options.comic=data;
...
$.ajax({dataType:"json",type:"POST",url:this.options.baseHref+"Comic/PublishWizard",data:$.postify(data),success:function(data,textStatus,request){if(data){self.options.comic=data;
...
},rate:function(source,action){this.requestConnect(false,function(){var count=$(".readerCount","label[for="+source.id+"]").text();
if(count=="-"){count=0;
...
$.ajax({dataType:"json",type:"POST",url:this.options.baseHref+"Comic/"+action,data:$.postify(args)});
});
